...
namespace AutomationSystem.Main.Web
{
    public partial class Startup
    {
        ...
        public void ConfigureAuth(IAppBuilder app)
        {
            ...
            app.CreatePerOwinContext<IAppUserDatabaseLayer>(AppUserDatabaseLayer.Create);
            app.CreatePerOwinContext<ApplicationUserManager>(ApplicationUserManager.Create);
            app.CreatePerOwinContext<ApplicationSignInManager>(ApplicationSignInManager.Create);

            // singletons from FactoryProviders
            var builder = IocProvider.Get<IClaimsIdentityBuilder>();
                ...
                Provider = new CookieAuthenticationProvider
                {
                    ...
                    OnValidateIdentity = SecurityStampValidator.OnValidateIdentity<ApplicationUserManager, ApplicationUser, int>(
                        ...
                        regenerateIdentityCallback: (manager, user) =>
                            user.GenerateUserIdentityAsync(manager, builder),
                        getUserIdCallback: (id) => (id.GetUserId<int>()))
                }
            });
            app.UseExternalSignInCookie(DefaultAuthenticationTypes.ExternalCookie);

            // sets google authentication
